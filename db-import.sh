...
echo "Usage by month:"
source .config
sqlite3 -column -header hep.db "with x as (select datetime(p.datum, 'start of month') as mjesec, sum(p.energija) as P_kwh, sum(r.energija) as R_kwh from p join r on p.datum=r.datum and p.vrijeme = r.vrijeme where p.datum >= '${START_MONTH}' group by 1 order by 1 asc) select *, (P_kwh-R_kwh) as razlika_kwh from x;"
